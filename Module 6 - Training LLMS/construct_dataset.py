...
def construct_dataset(data_txt_file, sequence_length=256):
    ...
    # tokenize every sample and append eos, then concatenate into one long stream
    all_tokens = []
    ...
    # pack into fixed-length chunks of (sequence_length + 1)
    # the +1 lets us use data[:, :-1] as input and data[:, 1:] as targets
    chunk = sequence_length + 1
    num_sequences = len(all_tokens) // chunk
    all_tokens = all_tokens[: num_sequences * chunk]

    data = np.array(all_tokens, dtype=np.int32).reshape(num_sequences, chunk)

    # shuffle
    np.random.shuffle(data)

    np.save("./dataset.npy", data)
    print(f"Saved {num_sequences} sequences of length {chunk} to dataset.npy")
