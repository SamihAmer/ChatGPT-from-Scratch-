"""
construct_dataset.py - tokenize a text corpus and pack it into a numpy array
of fixed-length training windows.

The output shape is (num_sequences, sequence_length + 1). The +1 lets the
training loop slice it into (input, target) pairs for next-token prediction.
...
def main():
    ...
    ap.add_argument("--text", required=True, help="Plain-text corpus, one doc per line.")
    ap.add_argument("--tokenizer", required=True, help="Directory of a trained tokenizer.")
