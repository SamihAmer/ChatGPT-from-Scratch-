'''
Device-agnostic training loop for the GPT model.

Adapted from Module 6's train_model.py. Changes from the module version:
  - auto-selects device: CUDA -> MPS (Apple Silicon) -> CPU
  - takes dataset / output paths via argparse so the same script trains both
    the "general" and "medical" variants without code changes
  - logs (step, tokens_seen, chars_seen, loss) to a CSV so we can plot any
    x-axis later (chars_seen is important because the medical tokenizer packs
    more characters per token; "same steps" and "same tokens" are not the same
    as "same text exposure")
  - saves final weights and intermediate loss-curve PNG to the output dir

Usage:
  python train.py \
      --dataset artifacts/datasets/pubmed_train_medical.npy \
...
def _estimate_chars_per_token(tokenizer_dir, dataset, n_probe=500):
    '''
    Decode a small sample of packed sequences back to text to get an average
    characters-per-token ratio for this tokenizer+corpus pair. We use this to
    translate "tokens seen" into "chars seen" for honest cross-tokenizer plots.
    '''
    ...
    for epoch in range(args.epochs):
        for (batch,) in loader:
            ...
            if step % args.log_interval == 0:
                ...
                print(f"step {step:>6} | tokens {tokens_seen:>12,} | chars {int(chars_seen):>14,} | loss {loss.item():.4f}")
